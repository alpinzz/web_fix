<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col">
            <span class="text-xs font-bold text-red-600 uppercase tracking-[0.2em] mb-1">Human Resources</span>
    ...
    <div class="animate-fade-in pb-12">
        <form action="{{ route('admin.members.store') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                        ...
                        <div class="flex flex-col items-center">
                            <div class="relative group">
                                <div
                                    ...
                                    <p class="text-[9px] font-black uppercase tracking-widest text-gray-400">Upload Foto
                                    </p>
                                </div>
                ...
                <div class="lg:col-span-2 space-y-10">
                    <div class="glass-card bg-white border border-gray-100 p-10 shadow-premium rounded-[2.5rem]">
                        <h3 class="text-xl font-black text-gray-900 tracking-tight mb-8">Identitas & Jabatan</h3>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <x-text-input id="name" type="text" name="name" :value="old('name')" required
                                    autofocus placeholder="Contoh: Ahmad Fauzi" />
                            ...
                            <div>
                                ...
                                <select id="division" name="division"
                                    ...
                                    <option value="" disabled {{ old('division') ? '' : 'selected' }}>Pilih Divisi
                                    </option>
                                    <option value="BPH" {{ old('division') == 'BPH' ? 'selected' : '' }}>BPH (Badan
                                        Pengurus Harian)</option>
                                    <option value="Bidang Kader"
                                        {{ old('division') == 'Bidang Kader' ? 'selected' : '' }}>Bidang Kader</option>
                                    <option value="Bidang Organisasi"
                                        {{ old('division') == 'Bidang Organisasi' ? 'selected' : '' }}>Bidang Organisasi
                                    </option>
                                    <option value="Bidang RPK" {{ old('division') == 'Bidang RPK' ? 'selected' : '' }}>
                                        Bidang RPK</option>
                                    <option value="Bidang Hikmah"
                                        {{ old('division') == 'Bidang Hikmah' ? 'selected' : '' }}>Bidang Hikmah
                                    </option>
                                    <option value="Bidang TKK" {{ old('division') == 'Bidang TKK' ? 'selected' : '' }}>
                                        Bidang TKK</option>
                                    <option value="Bidang SBO" {{ old('division') == 'Bidang SBO' ? 'selected' : '' }}>
                                        Bidang SBO</option>
                                    <option value="Bidang Medkom"
                                        {{ old('division') == 'Bidang Medkom' ? 'selected' : '' }}>Bidang Medkom
                                    </option>
                                </select>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updatePositions() {
                ...
                options.forEach(option => {
                    ...
                    if (option === oldPosition) opt.selected = true;
